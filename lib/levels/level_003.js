define([
  ...
  // Temporary solution i will remove soon . key will implement renderable
  // component which will be removed once it is collected
  function keyIsCollected(){
    var i;
    if(!player.inventory){
      return false;
    }
    for(i = 0; i < player.inventory.length; i += 1){
      if(player.inventory[i].name === key.name){
        return true;
      }
    }
    return false;
